...
export function AiChatPage() {
  ...
  return <div className="chat-page">
    ...
    <div className="messages">{messages.map((message, index) => <div className={`bubble ${message.role}`} key={index}>{message.role === 'assistant' ? <><Bot className="bubble-icon" /><MarkdownMessage content={message.content} /></> : message.content}</div>)}{busy && <div className="bubble assistant typing">● ● ●</div>}</div>
