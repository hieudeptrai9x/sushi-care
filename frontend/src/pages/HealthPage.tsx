...
export function HealthPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    await api.post('/api/activities/create.php', {
      ...
      weight_kg: tab === 'weight' ? value : undefined,
      temperature: tab === 'temperature' ? value : undefined,
      meta: { position }, note,
    })
  ...
  return <div className="page-pad">
    ...
    <form className="card entry-form" onSubmit={submit}>
      {tab === 'weight' && <Field label="Cân nặng (kg)"><input type="number" step="0.01" min="0.5" max="30" value={value} onChange={(e) => setValue(e.target.value)} required /></Field>}
      {tab === 'temperature' && <><Field label="Nhiệt độ (°C)"><input type="number" step="0.1" min="30" max="45" value={value} onChange={(e) => setValue(e.target.value)} required /></Field><Field label="Vị trí đo"><select value={position} onChange={(e) => setPosition(e.target.value)}><option value="armpit">Nách</option><option value="ear">Tai</option><option value="forehead">Trán</option><option value="rectal">Hậu môn</option></select></Field></>}
